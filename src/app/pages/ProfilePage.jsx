...
import { User, Mail, Lock, Upload, Trash2, X } from 'lucide-react';
...
export function ProfilePage() {
    ...
    // Sync form eenmalig zodra user geladen wordt na refresh
...
const handleUpdateProfile = async (e) => {
    ...
    if (response.ok) {
        ...
        updateUser({ username: data.userName, email: data.email, profilePicture: profilePicture ?? user?.profilePicture });
        setFormData((prev) => ({ ...prev, username: data.userName, email: data.email }));
        setSavedData({ username: data.userName, email: data.email, profilePicture: profilePicture ?? user?.profilePicture });
    ...
    const handleChangePassword = async (e) => {
        ...
        if (response.ok) {
            ...
            const msg = errors?.[0]?.description ?? "Password change failed";
            toast.error(msg);
        }
    };
